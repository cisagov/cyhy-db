...
async def test_connection_pymongo(db_uri, db_name):
...
@patch("cyhy_db.db.AsyncMongoClient")  # mock_async_mongo_client
@pytest.mark.asyncio
async def test_initialize_db_success(mock_async_mongo_client, mock_init_beanie):
    """Test a success case of the initialize_db function."""
    db_uri = "mongodb://localhost:27017"
    db_name = "test_db"

    mock_client = AsyncMock()
    mock_db = AsyncMock(AsyncDatabase)
    mock_client.__getitem__.return_value = mock_db
    mock_async_mongo_client.return_value = mock_client

    db = await initialize_db(db_uri, db_name)
    assert db == mock_db
    mock_async_mongo_client.assert_called_once_with(db_uri)
...
async def test_initialize_db_failure():
    ...
    with patch(
        "cyhy_db.db.AsyncMongoClient", side_effect=Exception("Connection error")
    ):
